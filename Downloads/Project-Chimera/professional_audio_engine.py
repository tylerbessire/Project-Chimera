...
class ProfessionalAudioEngine:
    ...
    def _process_mashup_section(self, audio_a: AudioBuffer, audio_b: AudioBuffer, 
        ...
        if section['source'] == 'a':
            ...
            audio_slice = np.tile(audio_slice, (1, repeat_times))
            ...
            if section.get('crossfade', False):
                ...
                bpm = section.get('bpm')
                mixed = self._crossfade_audio(bed_track, vocal_track,
                                             fade_duration=fade_dur,
                                             bpm=bpm)
                mixed = np.tile(mixed, (1, repeat_times))
                return mixed
            else:
    ...
    def _crossfade_audio(self, audio_a: np.ndarray, audio_b: np.ndarray,
                        ...
                        bpm: Optional[float] = None) -> np.ndarray:
        """Apply high-quality equal-power crossfade between two segments.

        If ``bpm`` is provided the fade duration is adjusted to span roughly
        four beats. This produces smoother musical transitions that better
        match the emotional pacing of the songs.
        """
