...
class EnhancedMashupCreator:
    ...
    def create_professional_mashup(self, song_a_path: str, song_b_path: str,
                                 ...
                                 mashup_recipe: Optional[dict] = None) -> MashupProject:
        """
        ...
        try:
            ...
            # Attach user-provided recipe structure (if any)
            if mashup_recipe and 'structure' in mashup_recipe:
                mashup_analysis.sections = self._parse_recipe_structure(mashup_recipe['structure'])
    ...
    def _create_professional_mashup(self, song_a_path: str, song_b_path: str,
        ...
        # Inject AI transition points into analysis for the audio engine
        if transition_plan and hasattr(transition_plan, 'transition_points'):
            analysis.transition_points = transition_plan.transition_points
    ...
    def _parse_recipe_structure(self, structure_dict: dict) -> list:
        """Convert nested mashup_recipe['structure'] dict into flat list of section dicts
        compatible with ProfessionalAudioEngine._convert_recipe_to_structure."""
        sections = []
        for label, info in structure_dict.items():
            duration_str = info.get('duration', '')
            # Convert duration like '0:00-1:30' to seconds length
            if '-' in duration_str:
                start_str, end_str = duration_str.split('-')
                def _ts(val):
                    mins, secs = val.split(':') if ':' in val else ('0', val)
                    return int(mins)*60 + float(secs)
                start_sec = _ts(start_str)
                end_sec = _ts(end_str)
                duration = end_sec - start_sec
            else:
                # fallback assume mm:ss
                try:
                    mins, secs = duration_str.split(':')
                    duration = int(mins)*60 + float(secs)
                except Exception:
                    duration = 16.0
            sections.append({
                'section_label': label,
