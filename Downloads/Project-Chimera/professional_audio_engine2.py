...
class ProfessionalAudioEngine:
    ...
    def _convert_recipe_to_structure(self, recipe_sections, audio_a: AudioBuffer, audio_b: AudioBuffer) -> List[Dict]:
        """Convert Luna/Claude recipe sections to engine-friendly structure.
        This version respects which track each section should emphasize and avoids
        hard‑coded crossfades that smother secondary material."""
        structure = []
        current_time = 0
        for section in recipe_sections:
            duration = section.get("duration", 16)
            # Decide source: if elements reference only one artist, pick that track.
            elements = section.get("elements", {})
            has_blunt = any("blunt" in k.lower() for k in elements.keys())
            has_killers = any("killer" in k.lower() for k in elements.keys())
            if has_blunt and not has_killers:
                source = "a"
            elif has_killers and not has_blunt:
                source = "b"
            else:
                source = "both"
            structure.append({
                ...
                "duration": duration,
                "crossfade": False,  # We'll let the engine mix naturally
                "recipe_data": section
            })
